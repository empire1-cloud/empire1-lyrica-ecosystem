...
const plans = [
  ...
  { name: "Pro", price: "$49", suffix: "/seat/month", note: "For founders and technical leads", features: ["500 tasks each month", "AI + rule fallback", "Reports, ROI and PR previews"], cta: "Start Pro", featured: true },
  { name: "Team", price: "$299", suffix: "/workspace/month", note: "10 seats included", features: ["5,000 tasks each month", "GitHub, Slack and Linear hooks", "Audit export and priority support"], cta: "Book a team setup" },
  ...
  return (
    <main>
      ...
      <section className="pricing" id="pricing">
        ...
        <div className="plans">{plans.map((plan) => <article className={plan.featured ? "featured" : ""} key={plan.name}>{plan.featured && <small>BEST START</small>}<h3>{plan.name}</h3><div className="price">{plan.price}<span>{plan.suffix}</span></div><p>{plan.note}</p><ul>{plan.features.map(f => <li key={f}>{f}</li>)}</ul><a href={`mailto:${email}?subject=${encodeURIComponent(`Omni-Agent ${plan.name}`)}`}>{plan.cta}</a></article>)}</div>
